...
struct Settings {
  ...
  // clusterRange için varsayılan 0.94 çok sıkıydı, özellikle gürültülü
  // ortamlarda. 0.88f ile daha esnek ve gerçekçi sonuçlar alınabilir.
...
inline Settings load_settings() {
  ...
  s.model_filename =
      get_env("STT_WHISPER_SERVICE_MODEL_FILENAME", "ggml-" + size + ".bin");

  s.vad_model_filename =
      get_env("STT_WHISPER_SERVICE_VAD_MODEL", s.vad_model_filename);
  ...
  s.vad_threshold =
      get_float("STT_WHISPER_SERVICE_VAD_THRESHOLD", s.vad_threshold);
  s.vad_ms_min_duration =
      get_int("STT_WHISPER_SERVICE_VAD_MS_MIN_DURATION", s.vad_ms_min_duration);
  ...
  s.enable_diarization =
      get_bool("STT_WHISPER_SERVICE_ENABLE_DIARIZATION", s.enable_diarization);
  s.cluster_threshold =
      get_float("STT_WHISPER_SERVICE_CLUSTER_THRESHOLD", s.cluster_threshold);

  s.n_threads = get_int("STT_WHISPER_SERVICE_THREADS", s.n_threads);
  s.parallel_requests =
      get_int("STT_WHISPER_SERVICE_PARALLEL_REQUESTS", s.parallel_requests);
  s.request_queue_timeout_ms = get_int("STT_WHISPER_SERVICE_QUEUE_TIMEOUT_MS",
                                       s.request_queue_timeout_ms);
  ...
  // Bu kısım UI da api lerde yada grpc de bir parametre olarak kullanıyormu?
  s.logprob_threshold =
      get_float("STT_WHISPER_SERVICE_LOGPROB_THRESHOLD", s.logprob_threshold);
  // [ARCH-COMPLIANCE FIX]: Infra'dan pozitif gelse bile logprob daima negatif
  // olmalıdır.
  if (s.logprob_threshold > 0.0f) {
    s.logprob_threshold = -s.logprob_threshold;
  }

  s.no_speech_threshold = get_float("STT_WHISPER_SERVICE_NO_SPEECH_THRESHOLD",
                                    s.no_speech_threshold);

  s.sample_rate = get_int("STT_WHISPER_SERVICE_SAMPLE_RATE", s.sample_rate);

  // [YENİ]: Çevresel değişkenden oku
  s.stream_buffer_samples = get_int("STT_WHISPER_SERVICE_STREAM_BUFFER_SAMPLES",
                                    s.stream_buffer_samples);
