...
void AudioClient::transcribe_file(const std::string& filepath) {
  ...
  try {
    ...
    grpc::Status status =
        stub_->WhisperTranscribe(&context, request, &response);

    if (status.ok()) {
      spdlog::info("✅ Transcription Success!");
      spdlog::info("Text: {}", response.transcription());
      spdlog::info("Lang: {} (Prob: {:.2f})", response.language(),
...
void AudioClient::transcribe_stream(const std::string& filepath) {
  ...
  try {
    auto audio_data = read_wav_body(filepath);
    size_t chunk_size =
        16000 * 2 * 0.5;  // 0.5 saniyelik chunklar (16kHz * 16bit)
    size_t offset = 0;
    ...
    while (offset < audio_data.size()) {
      size_t current_chunk_size =
          std::min(chunk_size, audio_data.size() - offset);
      ...
      std::this_thread::sleep_for(
          std::chrono::milliseconds(500));  // Gerçek zamanlı simülasyon
      std::cout << "." << std::flush;
    }
    ...
    while (stream->Read(&resp)) {
      spdlog::info("Stream Recv: {} (Final: {})", resp.transcription(),
                   resp.is_final());
    }
